...
参考 obra/superpowers 实战沉淀。写完技能初稿后，**作者自己**过一遍这 5 步，别等用户测出来。
...
技能里带 `scripts/` 的，**别写完就交**——先在自己机器上真跑一遍：

- 拿真实输入跑 happy path，确认输出和预期一致（不是"大概对"）
- **再故意踩坑**：参数给错、文件不存在、输入为空或格式不对。不许只有 happy path 能跑——用户第一件事就是把路径打错
- 报错要能看懂：说清哪个文件、什么问题、怎么改。别甩一个裸 traceback 就完事
- 路径别写死绝对路径，从参数或平台给的资源目录取
- 真跑不起来就**退回轻量流程**把步骤写清楚，别交一个跑不起来的脚本

**为什么这条必须你自己做**：`quick_validate` 只做语法与合规检查（`ast` 解析 + 网络/敏感信息正则），**不执行脚本、不做 import 检查**。脚本逻辑错、依赖没装，工具链一个字都不会说，只有你跑过才知道。
